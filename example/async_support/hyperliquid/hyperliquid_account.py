...
async def main():
    ...
    async with Client(testnet=True) as client:
        ...
        result = await client.user_rate_limit(
            user= wallet_address
        )
        print(result)

        result = await client.order_status(
            user= wallet_address,
            oid=32755192478
        )
        print(result)

        result = await client.historical_orders(
            user= wallet_address
        )
        print(result)

        result = await client.subaccounts(
            user= wallet_address
        )
        print(result)

        result = await client.user_role(
            user= wallet_address
        )
        print(result)
